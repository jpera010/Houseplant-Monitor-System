...
void Security()
{
   ...
   // Identify fingerprint test
    if (fps.IsPressFinger())
    {
      ...
      if (id <200)
      {
        ...
        //Serial.println(id);
        //Labeling specific ID's to the fingerprints that they are connected to
        Serial.print("App Notification: ");
        if(id == 3)
        {
          Serial.print("Akhil's right index finger");
        }
        else if(id == 4)
        {
          Serial.print("Akhil right middle finger");
        }
        else if(id == 5)
          {
            Serial.print("Akhil right ring finger");
          }
          Serial.println(" was used to open the system");
        ...
        {//if unable to recognize
          Serial.println("Finger not recognized, no entry");
          delay(500);
        }
      }
      else
  ...
  {
    delay(1000);
    Serial.print("System closing in ");
  ...
  void loop()
  {
  	while(!stop)
    {
      Security(); //Function that runs when the system is open
    }
    while(stop)
    {
      Closed(); //Mock test for closed system
      countdown--; //System will be closed for 3 seconds based on this mock test
      if(countdown == 0)
      {
        stop = false; //boolean that changes based on whether the system is open or closed
      }
    }
}
