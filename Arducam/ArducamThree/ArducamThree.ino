...
// connect to wifi.
  ...
  unlocked = !(Firebase.getInt("tools/lock"));
  if (!unlocked) {
...
void loop(){
  ...
  signalDetected = Firebase.getInt("alerts/updateSensors"); 
...
void updateTemp(int temp, int tempAlert) {
  ...
  Firebase.setInt("alerts/updateSensors", 0); 
  if (Firebase.failed()) {
